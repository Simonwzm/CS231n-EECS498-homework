...
def rnn_step_backward(dnext_h, cache):
    ...
    f4dtanh = lambda x: 4/(torch.exp(x)+torch.exp(-x))**2
    dtanh = f4dtanh(_x) * dnext_h
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # T controls the number of cascading timestamps


#-----------------------------------------------------------------
    # error code that is incompatible with pytorch autograd
    # N,T,D = x.shape
    # H = h0.shape[1]
    # # h = torch.zeros((N,T+1,H), dtype=x.dtype, device=x.device)
    # h = torch.zeros((N,T+1,H), dtype=x.dtype, device=x.device)
    # h[:,0,:] = h0
    # step_cache = tuple()
    # step_cache_list = []
    ...
    #     h[:,i+1,:],step_cache= rnn_step_forward(x[:,i,:], h[:,i,:], Wx, Wh, b)

    #     step_cache_list.append(step_cache)
    # h = h[:,1:,:]
#-----------------------------------------------------------------

    # no inplace computation, compatible for autograd
    N,T,D = x.shape
    H = h0.shape[1]
    h_list = []
    h_list.append(h0)
    ...
    for i in range(0,T, 1):
        h_next,step_cache= rnn_step_forward(x[:,i,:], h_list[-1], Wx, Wh, b)
        h_list.append(h_next)
        step_cache_list.append(step_cache)

    h = torch.stack(h_list[1:], dim=1)  # convert the list of tensors into a tensor

    cache = (x, h0, Wx, Wh, b, step_cache_list)
...
def rnn_backward(dh, cache):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x, h0, Wx, Wh, b, step_cache_list = cache
    N,T,D = x.shape
    H = h0.shape[1]
    ...
    # x,prev_h,Wx,Wh,b,_x = cache
    dprev_dh_i2 = torch.zeros_like(h0)
    for i in range(T-1, -1, -1):
        dh_i1 = dh[:,i,:]
        # dx_i = dx[:,i,:]
        dh_i = dprev_dh_i2 + dh_i1
        
        dx_i, dprev_dh_i2, dWx_i, dWh_i, db_i = rnn_step_backward(dh_i, step_cache_list[i])
        dWx += dWx_i
        dWh += dWh_i
        db += db_i
        dx[:,i,:] = dx_i
...
# not vectorized
#-----------------------------------------------------------------
        # out = torch.zeros(x.shape[0], x.shape[1], self.W_embed.shape[1], device=x.device)
        # # print(x)
        # for i in range(x.shape[0]):
        #     for j in range(x.shape[1]):
        #         out_ij = self.W_embed[x[i,j]]
        #         out_new = out.clone()
        #         out_new[i,j] = out_ij
        #         out = out_new
        # out = out.cuda()
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        self.imge_layer = ImageEncoder(image_encoder_pretrained,True)
        self.wdemb_layer = WordEmbedding(vocab_size, wordvec_dim)
        self.rnn_layer = RNN(wordvec_dim, hidden_dim)
        self.featProjLine_layer = nn.Linear(input_dim, hidden_dim)
        self.score_layer = nn.Linear(hidden_dim, vocab_size)
    ...
    def forward(self, images, captions):
        ...
        # if self.cell_type == 'rnn':
        h0 = None
        feat = self.imge_layer.forward(images)
        feat = F.avg_pool2d(feat,kernel_size=(feat.shape[2], feat.shape[3]))
        feat = torch.flatten(feat,start_dim=1)
        feat_proj = self.featProjLine_layer.forward(feat)
        h0 = feat_proj

        input_wd = self.wdemb_layer.forward(captions_in)

        if self.cell_type == 'rnn':
            all_hidden_out = self.rnn_layer.forward(input_wd, h0)
        elif self.cell_type == 'lstm':
            all_hidden_out = self.lstm_layer.forward(input_wd, h0)

        all_score_out = self.score_layer.forward(all_hidden_out)

        loss = temporal_softmax_loss(all_score_out, captions_out, self.ignore_index)
        # else:
        #     print('unfinished')
    ...
    def sample(self, images, max_length=15):
        ...
        # print(h.shape)
        # prev_wd_index = self._start
        _captions = torch.ones(images.shape[0], max_length+2).long() #no start no end
        _captions[:,0] = self._start
        for i in range(max_length):
            next_cap = self.wdemb_layer(_captions[:,i]) 
            if self.cell_type == 'rnn':
                hidden_out = self.rnn_layer.step_forward(next_cap, h)
            elif self.cell_type == 'lstm':
                hidden_out, cell_out = self.lstm_layer.step_forward(next_cap, h, c)
            score_out = self.score_layer(hidden_out)
            # next_cap = torch.argmax(score_out, dim=1, keepdim=True)
            _, next_cap = score_out.max(dim=1)
            h = hidden_out
            if self.cell_type == 'lstm':
                c = cell_out
            # print(next_cap)
            _captions[:,i+1] = next_cap
            # print(_captions)
            # _captions = _captions.to(torch.int64)
        captions = _captions[:,1:-1]
        # captions[0,0] = 2
        # else:
        #     print('unfinished')
