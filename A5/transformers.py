...
def scaled_dot_product_two_loop_single(
    ...
    for k_q in range(K):
        qi = query[k_q]
        e_list = torch.matmul(qi, key.transpose(0,1)).reshape(-1, 1) / M**(0.5)
        a_list = torch.softmax(e_list, dim=0)
        yi = torch.matmul(a_list.transpose(0,1), value).reshape(-1)
        out_list.append(yi)
...
def scaled_dot_product_two_loop_batch(
    ...
    # Replace "pass" statement with your code
    N, K, M = query.shape
    E_matrix = torch.bmm(query, key.transpose(1,2)).transpose(1,2) / M**(0.5)

    A_matrix = torch.softmax(E_matrix, dim=1)
    Y_matrix = torch.bmm(A_matrix.transpose(1,2), value)
    out=Y_matrix
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
def scaled_dot_product_no_loop_batch(
    ...
    if mask is not None:
        ...
        # Replace "pass" statement with your code
        N, K, M = query.shape
        E_matrix = torch.bmm(key,query.transpose(1,2)) / M**(0.5)
        mask = mask.to(E_matrix.device)
        E_matrix = torch.masked_fill(E_matrix, mask.transpose(1,2), -1e9)
        A_matrix = torch.softmax(E_matrix, dim=1)
        y = torch.bmm(A_matrix.transpose(1,2), value)
        weights_softmax = A_matrix.transpose(1,2)
        return y, weights_softmax
    # Replace "pass" statement with your code
    N, K, M = query.shape
    E_matrix = torch.bmm(key,query.transpose(1,2)) / M**(0.5)
    # E_matrix = E_matrix.to(mask.device)
    A_matrix = torch.softmax(E_matrix, dim=1)
    y = torch.bmm(A_matrix.transpose(1,2), value)
    weights_softmax = A_matrix.transpose(1,2)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        for att_layer in self.multi_head:
            out = att_layer.forward(query, key, value, mask)
            outputs.append(out)
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.line1 = nn.Linear(inp_dim, hidden_dim_feedforward)
        self.line2 = nn.Linear(hidden_dim_feedforward, inp_dim)
        c = (6/(inp_dim + hidden_dim_feedforward))**(0.5)
        # c_q = (6/(dim_in + dim_q))**(0.5)
        nn.init.uniform_(self.line1.weight, -c, c)
        nn.init.uniform_(self.line2.weight, -c, c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.multi_head_layer = MultiHeadAttention(num_heads, emb_dim, emb_dim//num_heads)
        self.layer_norm_layer1 = LayerNormalization(emb_dim)
        self.layer_norm_layer2 = LayerNormalization(emb_dim)
    ...
    def forward(self, x):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        out_multih = self.multi_head_layer.forward(x,x,x)
        # print(x.shape)
        # print(out_multih.shape)
        out_res1 = out_multih + x
        out_lynm1 = self.layer_norm_layer1.forward(out_res1)
        out_dp = self.dropout_layer(out_lynm1)
        out_ff = self.feedforward_layer.forward(out_dp)
        out_res2 = out_ff + out_dp
        out_lynm2 = self.layer_norm_layer2.forward(out_res2)
        y = self.dropout_layer(out_lynm2)
...
#     ##############################################################################
#     # Replace "pass" statement with your code
#     y = torch.tensor([n/K for n in range(K)], device='cuda')[None,:,None].repeat((1,1,M))
#     # print(y.shape)
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    # pe(p,i) = abs(sin(p/10000**a + 0.5pi*i))
    import math
    y = torch.zeros(1,K,M, device='cuda')
    for p in range(K):
        for i in range(M):
            a = math.floor(i/M)
