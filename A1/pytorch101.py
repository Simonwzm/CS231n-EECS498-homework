...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    # built_in method using torch.arange()
    start = (start // 10 + 1) * 10
    if start > stop:
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = x[torch.arange(x.shape[0]-1, -1, step=-1),:]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    for i in range(x.shape[0]): # for each batch
        results.append(torch.mm(x[i], y[i]))
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    sd = torch.sqrt(1/(x.shape[0]-1) * torch.sum((x-col_mean) * (x-col_mean), dim=0))
    y = (x-col_mean) / sd
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
