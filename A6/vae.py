...
def reparametrize(mu, logvar):
    ...
    # print(logvar)
    eps = torch.randn(size=(mu.shape[0],mu.shape[1]), device=mu.device)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    #NOTE!! cross entropy always return a scalar, and 'mean' reduction takes the mean over
    # all elements in the tenor. But we here only need to do mean inside each sample in
    # the batch, not across the batch. BE CAREFUL!!
    reconstruct_loss = nn.functional.binary_cross_entropy(x_hat, x, reduction='sum') / x.shape[0]
    batch_kl_loss = -0.5 * torch.sum((1+logvar - mu**2 - torch.exp(logvar)), dim=1)
    kl_loss = torch.sum(batch_kl_loss) / x.shape[0]
