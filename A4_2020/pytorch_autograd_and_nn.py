...
class ThreeLayerConvNet(nn.Module):
  ...
  #   1. Convolutional layer with channel_1 5x5 filters with zero-padding of 2  
    ...
    ############################################################################
    # Replace "pass" statement with your code
    self.cv1 = nn.Conv2d(in_channel, channel_1, 5, 1, 2)
    self.cv2 = nn.Conv2d(channel_1, channel_2, 3, 1, 1)
    ...
    nn.init.zeros_(self.cv1.bias)
    nn.init.zeros_(self.cv2.bias)
    ############################################################################
    #                           END OF YOUR CODE                            
    ############################################################################
...
def initialize_three_layer_conv_part4():
  ...
  model = nn.Sequential(OrderedDict([
    ('cv1', nn.Conv2d(C, channel_1, 5, 1, 2)),
    ('relu1', nn.ReLU()),
    ('cv2', nn.Conv2d(channel_1, channel_2, 3, 1, 1)),
    ('relu2', nn.ReLU()),
    ('flatten', Flatten()),
...
class PlainBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    my_model = nn.Sequential(OrderedDict([
      ...
      ('relu1', nn.ReLU()),
      ('conv1', nn.Conv2d(Cin, Cout, 3, stride, 1)),
      ('sbn2', nn.BatchNorm2d(Cout)),
      ('relu2', nn.ReLU()),
      ('conv2', nn.Conv2d(Cout, Cout, 3, 1, 1))
    ]))
    self.net = my_model
    ############################################################################
    #                                 END OF YOUR CODE                         #
...
class ResidualBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    if downsample:
      ...
      stride = 1
    main_model = nn.Sequential(OrderedDict([
      ('sbn1', nn.BatchNorm2d(Cin)),
      ('relu1', nn.ReLU()),
      ('conv1', nn.Conv2d(Cin, Cout, 3, stride, 1)),
      ('sbn2', nn.BatchNorm2d(Cout)),
    ...
    self.block = main_model

    sc_model = None
    if downsample:
      sc_model = nn.Sequential(nn.Conv2d(Cin, Cout, 1, 2))
    elif Cin == Cout:
      sc_model = nn.Sequential(nn.Identity()) 
    else:
      sc_model = nn.Sequential(nn.Conv2d(Cin, Cout, 1))
    self.shortcut = sc_model
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    stem_target = stage_args[0][0]
    stage_args = [(*i,block) for i in stage_args]
    resstem = ResNetStem(Cin, stem_target)
    resstage_list = [ResNetStage(*i) for i in stage_args]
...
class ResidualBottleneckBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.block = nn.Sequential(*main_layer)
